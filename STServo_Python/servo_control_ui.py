...
class ServoRow:
    ...
    def __init__(self, parent, root, servo_id, row, on_write):
        ...
        self.slider.bind("<ButtonRelease-1>", self._flush_now)
    ...
    def _on_slider(self, value_str):
        ...
        if self._write_job is not None:
            self._root.after_cancel(self._write_job)
        self._write_job = self._root.after(
            WRITE_DEBOUNCE_MS, lambda: self._flush(value)
        )

    def _flush(self, value):
        self._write_job = None
        self.on_write(self.servo_id, value)

    def _flush_now(self, _event):
        if self._write_job is not None:
            self._root.after_cancel(self._write_job)
            self._write_job = None
...
class ServoControlApp:
    def __init__(self, root):
        ...
        self.rows = [
            ServoRow(body, root, servo_id, i, self._write_position)
    ...
    def _connect(self):
        ...
        try:
            ...
            if not self._port.openPort():
                raise OSError("could not open %s" % port_name)
            if not self._port.setBaudRate(BAUDRATE):
                raise OSError("unsupported baudrate %d" % BAUDRATE)
            self._packet = sms_sts(self._port)
            self._group = GroupSyncRead(self._packet, SMS_STS_PRESENT_POSITION_L, 4)
            for servo_id in SERVO_IDS:
                self._group.addParam(servo_id)
        except Exception as exc:  # noqa: BLE001 - surface any open failure to the UI
            self._close_port()
            self._set_status("Connect failed: %s" % exc, False)
            return
    ...
    def _poll(self):
        ...
        try:
            # txRxPacket's global result is COMM_RX_CORRUPT whenever ANY id in
            # the group is missing, so it cannot decide per-servo status.
            # Per-servo availability comes from isAvailable(), exactly like the
            # SDK sync_read.py example.
            self._group.txRxPacket()
            for row in self.rows:
                servo_id = row.servo_id
                available, _error = self._group.isAvailable(
                    servo_id, SMS_STS_PRESENT_POSITION_L, 2
                )
                if available:
                    position = self._group.getData(
                        servo_id, SMS_STS_PRESENT_POSITION_L, 2
                    )
                    speed_raw = self._group.getData(
                        servo_id, SMS_STS_PRESENT_SPEED_L, 2
                    )
                    speed = self._packet.scs_tohost(speed_raw, 15)
                    row.set_feedback(position, speed, True)
                else:
                    row.set_feedback(None, None, False)
        except Exception:  # noqa: BLE001 - a broken bus must not crash the UI
            self._disconnect()
            self._set_status("Lost connection", False)
            return

        self._poll_job = self._root.after(POLL_INTERVAL_MS, self._poll)
    ...
    def _write_position(self, servo_id, position):
        if not self._connected:
            return
        try:
            self._packet.WritePosEx(servo_id, position, MOVE_SPEED, MOVE_ACC)
        except Exception:  # noqa: BLE001 - a dead write must not crash the UI
            self._disconnect()
            self._set_status("Write failed", False)
